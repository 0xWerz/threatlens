...
export default function Home() {
  ...
  useEffect(() => {
    async function loadPacks() {
      try {
        ...
        setPacks([{ id: "startup-default", name: "Startup Default", defaultFailOn: "high" }]);
        setScanMeta("Using fallback policy pack list.");
      }
    }
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setScanMeta(
        `Policy ${data.policy.id} · fail-on ${data.policy.failOn} · ${blockState} · deterministic high=${data.deterministicSummary?.high ?? 0}`,
      );
    } catch (error) {
  ...
  return (
    <>
      ...
      <header className="hero">
        <p className="eyebrow">PR Security Guardrails, Without Platform Bloat</p>
        <h1>
          ThreatLens
          <span>Next.js frontend for custom business-logic checks</span>
        </h1>
        <p className="hero-copy">
          This web app runs deterministic scans against the ThreatLens API. Private
          authenticated clients can enable advisory LLM mode server-to-server.
        </p>
      ...
      <main className="layout">
          ...
          <form className="scan-form" onSubmit={onSubmit}>
            ...
            <select
              ...
              onChange={(event) => {
                ...
                setPackId(next);
                const selected = packs.find((pack) => pack.id === next);
                if (selected) {
            ...
            <select
              ...
              name="failOn"
              value={failOn}
              onChange={(event) => setFailOn(event.target.value as "none" | Severity)}
            >
            ...
            <div className="actions">
              ...
              <button type="button" className="ghost" onClick={loadSample} disabled={isLoading}>
                Load sample
              </button>
            </div>
          </form>
        ...
        <section className="panel results-panel">
          ...
          <div className="findings">
            {findings.length === 0 ? (
              <div className="empty">No findings for this diff and policy pack.</div>
            ) : (
              findings.map((finding) => (
                <article key={`${finding.ruleId}-${finding.filePath}-${finding.line}-${finding.evidence}`} className={`finding ${finding.severity}`}>
                  <h4>
      ...
      <section className="proof-strip">
        ...
        <article>
          <h3>Private flow</h3>
          <p>
            Server-to-server clients can call authenticated scan endpoints with
            `x-api-key` and optional OpenRouter key override.
          </p>
        </article>
      </section>
    </>
